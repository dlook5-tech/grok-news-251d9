...
TOP (ALWAYS 3 stories): The 3 most viral posts on ALL of X from the LAST 6 HOURS. Any account. Must be FRESH. Return as array of 3. Keep body SHORT (1 sentence).

MSM (ALWAYS 3 stories): 3 stories blowing up on X that CNN/NYT/WaPo are ignoring. Return as array of 3. Rotate handles. Keep body SHORT. @BillMelugin_ @MattWalshBlog @TimcastNews @TheRabbitHole84 @SCOTUSblog @InsightGL @JamesOKeefeIII @LibsOfTikTok @TPostMillennial @RealSaavedra

PG6 (ALWAYS 3 stories): 3 hottest celebrity/entertainment gossip stories. Return as array of 3. Keep body SHORT. @PopCrave @enews @JustJared @etnow @TMZ

PODS (ALWAYS 3 clips from DIFFERENT shows): 3 viral podcast clips. Return as array of 3. Keep body SHORT. @joerogan @joeroganhq @TuckerCarlson @theallinpod @lexfridman @CallHerDaddy @adamcarolla @JREClips @enews @PBDPodcast @MegynKellyShow @fridmanclips

RECIPE (ALWAYS 3 stories): 3 actual FOOD RECIPES going viral. NOT politics. Must be about cooking/baking/food. Return as array of 3. Keep body SHORT. @tasteofhome @FoodNetwork @thekitchn @HBHarvest @foodandwine @tasty @KitchenSanc2ary @budgetbytes @halfbakedharvest @AmbitiousKitch

SCIENCE (ALWAYS 3 stories): 3 most viral science/tech/space/health posts. Return as array of 3. Keep body SHORT. @elikiml @ProfFeynman @BadAstronomer @ScienceAlert @NASAWebb @SPabortsev @EricTopol @NatureNews @SciAm @DrEricDing
